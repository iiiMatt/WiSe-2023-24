...
titanic_cleaned <- readRDS("titanic_cleaned.Rds")
...
cat("Age:
")
describe_metric(titanic_cleaned$Age)
...
SibSp
")
describe_metric(titanic_cleaned$SibSp)
cat("
Fare
")
describe_metric(titanic_cleaned$Fare)
cat("
Parch
")
describe_metric(titanic_cleaned$Parch)

#' 2a- ii)
#' descriptive statistics for a categorial variable
#' as an example we can use the categorial variable

describe_categorial(titanic_cleaned$Deck)
...
describe_categorial(titanic_cleaned$Pclass)
describe_categorial(titanic_cleaned$Survived)
describe_categorial(titanic_cleaned$Sex)
describe_categorial(titanic_cleaned$Embarked)
describe_categorial(titanic_cleaned$Salutation)
describe_categorial(titanic_cleaned$Side)
...
surv_embarked <- chisq_test_plus_table(titanic_cleaned$Survived, titanic_cleaned$Embarked)
surv_embarked[1]
"kat_var2
...
surv_salutation <- chisq_test_plus_table(titanic_cleaned$Survived, titanic_cleaned$Salutation)
surv_salutation[1]
"kat_var2
...
############################################################################
# survival and deck
sum(is.na(titanic_cleaned$Deck))
# too many NAs to proper analyze
############################################################################
# survival and Side
sum(is.na(titanic_cleaned$Side))
# too many NAs to proper analyze
...
calculateBivariateStats(titanic_cleaned, "Fare", "Survived")
...
calculateBivariateStats(titanic_cleaned, "Age", "Survived")
"$no
mean       sd
30.18488 12.65199
